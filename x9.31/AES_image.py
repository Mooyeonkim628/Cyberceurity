...
def ctr_aes_image(iv, image_file="image.ppm", out_file="enc_image.ppm", key_file="key.txt"):
    ...
    with open(image_file, "rb") as FILEIN2:
        message = (FILEIN2.readlines()[0:3])
    ...
    with open(out_file, 'wb') as FILEOUT:
        ...
        while bv.more_to_read:
            ...
            for i in range(1, 15):
                ...
                for row in range(4):
                    for column in range(4):
                        state[column][row] = enc[32 * row + 8 * column : 32 * row + 8 * (column + 1)]
            ...
            iv = BitVector(intVal=iv.int_val()+1, size=BLOCKSIZE)
