...
def encrypt():
    image = []
    with open(sys.argv[1], "rb") as FILEIN1:
        image = (FILEIN1.readlines()[0:3])
    bv = BitVector(filename=sys.argv[1])
    bv.read_bits_from_file(len(image) * 8)
    FILEIN1.close()
    ...
    with open(sys.argv[3], "wb") as FILEOUT:
        for i in image:
            FILEOUT.write(i)
    FILEOUT.close()
    ...
    while (bv.more_to_read):
        ...
        if bitvec.length() > 0:
            ...
            outputtext = RE + LE
            with open(sys.argv[3], 'ab') as fp:
                outputtext.write_to_file(fp)                                                                                                         
