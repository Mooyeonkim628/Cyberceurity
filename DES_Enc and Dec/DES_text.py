...
def substitute( expanded_half_block ): ##this funtions is referenced from Lecture 3 
...
def generate_round_keys(encryption_key): ##this funtions is referenced from Lecture 3 
    round_keys = []
...
def encrypt():
    FILEIN1 = open(sys.argv[2])      
    bv = BitVector( filename = sys.argv[2])    
    FILEIN1.close() 
    
    FILEIN2 = open(sys.argv[3])      
    key = BitVector( textstring = FILEIN2.read() )           
    ...
    while (bv.more_to_read):
        ...
        if bitvec.length() > 0:
            ...
            if bitvec.length() < BLOCKSIZE:
                bitvec = bitvec._resize_pad_from_right(BLOCKSIZE - bitvec.length())  
            ...
            for i in round_key:
                ...
                out_xor = newRE^i               
                re2 = substitute(out_xor)
                re2 = re2.permute(p_box)                            
                temp = RE
                RE = LE^re2
                LE = temp

            output = RE + LE
            outputhex = output.get_hex_string_from_bitvector()
            FILEOUT.write(outputhex)                                                                                                    

def decrypt():
    FILEIN1 = open(sys.argv[2])      
    bv = BitVector( hexstring = FILEIN1.read())    
    FILEIN1.close() 
    ...
    for i in range(0, len(bv) // BLOCKSIZE):
        ...
        if bitvec.length() > 0:
            ...
            outputtext = RE + LE
            outputtext.write_to_file(FILEOUT)                                                                                                          
